...
TEST_F(TestSpeedTest, test_speed)
{
	...
	for (uint32_t i = 0; i < NumBlocks; i++)
		magma_encrypt_block(round_keys, testA1A0);
	stop = chrono::high_resolution_clock::now();
	cout << endl
		 << "C++/C 10Mbyte encryption time: " << chrono::duration_cast<chrono::milliseconds>(stop - start).count() << " milliseconds" << endl;

	start = chrono::high_resolution_clock::now();
	for (uint64_t i = 0; i < NumBlocks; i++)
		encrypt(key, testA1A0);
	stop = chrono::high_resolution_clock::now();
	cout << endl
		 << "ASM 10Mbyte encryption time: " << chrono::duration_cast<chrono::milliseconds>(stop - start).count() << " milliseconds" << endl;
